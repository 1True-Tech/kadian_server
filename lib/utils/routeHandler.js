      ...
      try {
        ...
        if (result !== undefined) {
          ...
          if (Buffer.isBuffer(result) || result.pipe) {
            // streaming or buffer
            return res.send(result).status(result.statusCode || 200);
          } else {
            console.log(`${verb.toUpperCase()} ${path} : ${result.statusCode}`)

            return res.json(result).status(result.statusCode || 200);
          }
