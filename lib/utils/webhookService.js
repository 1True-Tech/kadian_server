...
class WebhookService {
   ...
   * @param {'user_created'| 'order_created' | 'order_status_changed' | 'account_updated'} eventType - Type of event (user_created, order_created, etc.)
   * @param {Object} eventData - Data related to the event
   * @returns {Promise<Object>} - Processing result
   */
  async processEvent(eventType, eventData) {
    try {
      logger.app.info({ eventType, eventData }, 'Processing webhook event');
      ...
      switch (eventType) {
        ...
        default:
          logger.app.warn({ eventType }, 'Unknown event type');
          return { success: false, message: 'Unknown event type' };
      }
    } catch (error) {
      logger.app.error({ error: error.message, eventType }, 'Error processing webhook event');
  ...
  async handleOrderCreated(data) {
    const { orderId, userId,customerInfo,items,total,status } = data;
    const results = [];
    ...
    const customerEmailResult = await sendNotificationEmail({
      ...
      data: {
        name: `${customerInfo.name.first} ${customerInfo.name.last}` || 'Valued Customer',
        orderId,
        total
      }
    });
    ...
    const adminEmailResult = await sendNotificationEmail({
      ...
      data: {
        orderId: orderId,
        userName: `${customerInfo.name.first} ${customerInfo.name.last}`,
        total
      }
    });
    results.push(adminEmailResult);
